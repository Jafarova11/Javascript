...
//1)
let data1 = countries_data.sort((a, b) => b.name.localeCompare(a.name));
console.log(data1);
...
//4)
let isEnglish = countries_data.filter((el) => el.languages.includes("English"));
console.log(isEnglish);

//5)
let isAsiaOrEurope = countries_data.filter(
  (el) => el.region == "Asia" || el.region == "Europe"
);
console.log(isAsiaOrEurope);

//6)
let isMoreLanguages = countries_data.filter((el) => el.languages.length > 1);
console.log(isMoreLanguages);

//7)
let countryAndCapital = countries_data.map(
  (el) => `${el.name} : ${el.capital}`
);
console.log(countryAndCapital);

// //8)

console.log(countries_data.reduce((acc, sum) => (acc += sum.population), 0));
